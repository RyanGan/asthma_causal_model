...
meow2 <- iptw_func(data=simdata, y="Y", a="A", w="W1+W2", estimate="diff")
meow2
```
...
```{r sl msm}
fev_msm_sl <- mean(expected_y_a1) - mean(expected_y_a0)
fev_msm_sl
```
...
```{r tmle step 4}
tmle_est <- mean(tmle_y_a1) - mean(tmle_y_a0)
tmle_est
```
...
```{r comparsion}
msm_fev <- cbind(meow, meow2, fev_msm_sl, tmle_est)
...
Wow. This was much faster and produced 95% CIs. Not even sure if it bootstrapped... Need to take a look at package description in J of Stats Software.
